...
def ntu_det_postproc(vid, det_results):
    ...
    #수정-------------------
    file_name = vid.split('/')[-1]  # 파일 이름 추출
    label_part = file_name.split('_')[-1]  # 'M1A001.mp4' 추출  
    label = label_part[1:4]  # 'A001' 추출 (이 부분은 파일 형식에 따라 조정 필요)
    #----------------------------------
...
#수정----------------------------------------------
def pose_inference_with_align(pose_config, pose_checkpoint, device, frame_paths, det_results):
    # filter frame without det bbox
    det_results = [
        frm_dets for frm_dets in det_results if frm_dets.shape[0] > 0
    ]

    pose_results, _ = pose_inference(pose_config, pose_checkpoint, frame_paths, det_results, device)
#------------------------------------------------------------   
...
#수정----------------------------------------------
def ntu_pose_extraction(vid, device, det_config, det_checkpoint, det_score_thr, skip_postproc=False):
    tmp_dir = TemporaryDirectory()
    frame_paths, _ = frame_extract(vid, out_dir=tmp_dir.name)
    det_results, _ = detection_inference(
        det_config,
